...
//            isRunning = false;
    ...
    private Runnable runnable_MainActivity = new Runnable() {
        ...
        public void run() {
            if (!isStart) {
                Log.e(TAG, "[run] runnable pause");
                return;
            }
            List<TAGINFO> list1;
            if (isMulti) {
                Log.e(TAG, "runnable-isMulti-true");
...
//            Log.e(TAG,"readtime:"+readtime+"read:"+read+"state:"+statenvtick);
            if (readtime > timeout) {
                MainActivity.mUhfrManager.asyncStopReading();
        ...
        switch (v.getId()) {
            ...
            case R.id.button_export:
                if (listEpc != null && listEpc.size() != 0) {
                    save(FileName());
                    Toast.makeText(getContext(), getString(R.string.success) + listEpc.size(), Toast.LENGTH_SHORT).show();
                } else {

...
{
    public class VHSTarjaAddDetailViewModel : BaseViewModel
    {
        ...
        private Models.AppModelVHSTarjaDetalle _currentEntry;
        public Models.AppModelVHSTarjaDetalle CurrentEntry
        {
            get => _currentEntry;
            set
            {
        ...
        public VHSTarjaMensaje VHSTarja { get; set; }
        public VHSTarjaAddDetailViewModel(VHSTarjaMensaje entry)
        {
            Title = "Agregar detalle Tarja";
            AddCommand = new Command(OnCrearClicked);
            CancelCommand = new Command(OnCancelClicked);
            if (CurrentEntry == null) CurrentEntry = new Models.AppModelVHSTarjaDetalle();
            VHSTarja = entry;
            ...
            try
            {
                ...
                if (flags == false)
                {
                    ...
                    if (current == NetworkAccess.Internet)
                    {
                        ...
                        request.TarjaID = this.VHSTarja.TarjaId;
                        ...
                        var response = await ServiceVHS.AddTarjaDetail(request);

    ...
    public class EditDetailViewModel : BaseViewModel
    {
        ...
        private async void GetRecepcion(string idSeleccioando)
        {
            try
            {
                ...
                if (current == NetworkAccess.Internet)
                {
                    ...
                    MyAgenda = await datos.GetRecepcion(0, long.Parse(idSeleccioando), userName).ConfigureAwait(true);
        ...
        private async Task CargaDetalle(recepcion oRec)
        {
            if (oRec != null)
            {
                ...
                temperature = "Commodity: " + _selectedItem.producto?.nombre;
                types = "Qty: " + _selectedItem.cantidad;
                comentary = "Planned Location: " + _selectedItem.Ubicaciones?.nombre;
                ...
                if (DeACuerdo)
                {
                    ...
                    if (flags == false)
                    {
                        ...
                        if (current == NetworkAccess.Internet)
                        {
                            ...
                            oRecepcion.lugar = _lugar;
                            oRecepcion.usuarioCrea = userName;

                            ApiModels.AppModels.Base msg = await datos.RegistraRecepcion(oRecepcion, ArrayFoto, ArrayFoto1, ArrayFoto2, ArrayFoto3).ConfigureAwait(true);
